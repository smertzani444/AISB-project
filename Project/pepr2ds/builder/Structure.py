...
class Structure(Attributes):
    ...
    def clean_pdb(self, pdb_path):
        """
        Clean a PDB file (remove hydrogens, alternate residues, rename terminus atoms,
        add a HEADER if missing) so that DSSP and FreeSASA both run without errors.
        Writes the cleaned file to the corresponding …/cleaned/ subfolder.
        """
        # 1) Read the raw PDB
        ...
        outpath = pdb_path.replace(os.sep + "raw" + os.sep,
                                os.sep + "cleaned" + os.sep)
        os.makedirs(os.path.dirname(outpath), exist_ok=True)

        cleaned = []
        saw_header = False
        # pre-compile the regex once
        h_regex = re.compile(r'^[\sA-Za-z0-9]*H', re.IGNORECASE)

        for line in lines:
            # track if there's already a HEADER
            if line.startswith("HEADER"):
                saw_header = True

            # keep any non-ATOM/HETATM record
            if not (line.startswith("ATOM") or line.startswith("HETATM")):
                cleaned.append(line)
                continue

            # parse the element (cols 77–78)
            element = line[76:78].strip()
            # parse the atom name (cols 13–16)
            atom_name = line[12:16]

            # drop any hydrogen if either:
            #  • element column says "H", OR
            #  • atom name (first non-space char) is 'H' after any prefix
            if element.upper() == "H" or h_regex.match(atom_name):
                continue

            # remove alternate-location indicator (col 17)
            if line[16] != " ":
                line = line[:16] + " " + line[17:]

            # remove insertion code (col 27)
            if line[26] != " ":
                line = line[:26] + " " + line[27:]

            # rename OXT → O   (cols 13–16)
            if line[12:16] == "OXT ":
                line = line[:12] + "O   " + line[16:]

            cleaned.append(line)

        # 3) If we never saw a HEADER, insert a minimal one at the top
        if not saw_header:
            insert_at = next((i for i, L in enumerate(cleaned) if L.strip()), 0)
